...
class FlxUIDropDownMenuCustom extends FlxUIGroup implements IFlxUIWidget, IFlxUIClickable, IHasParams
{
			...
			?DropPanel:FlxUI9SliceSprite, ?ButtonList:Array<FlxUIButton>, ?UIControlCallback:Bool->FlxUIDropDownMenuCustom->Void)
	{
		...
		if (DataList != null)
		{
			for (i in 0...DataList.length) list.push(makeListButton(i, DataList[i].label, DataList[i].name));
		...
		add(dropPanel);
		for (btn in list) { add(btn); btn.visible = false; }
	...
	private function updateButtonPositions():Void
	{
		...
		for (i in 0...currentScroll) if(list[i] != null) list[i].y = -99999;
		for (i in currentScroll...list.length)
		{
			var button:FlxUIButton = list[i];
			if(button != null) { button.y = offset; offset += buttonHeight; }
		}
	}
	...
	override function set_visible(Value:Bool):Bool
	{
		...
		var vButtons = [];
		for (i in 0...list.length) vButtons.push(list[i] != null ? list[i].visible : false);
		super.set_visible(Value);
		dropPanel.visible = vDropPanel;
		for (i in 0...list.length) if(list[i] != null) list[i].visible = vButtons[i];
		return Value;
	}

	private function dropsUp():Bool { return dropDirection == Up || (dropDirection == Automatic && exceedsHeight()); }
	private function exceedsHeight():Bool { return y + getPanelHeight() + header.background.height > FlxG.height; }
	private function getPanelHeight():Float { return list.length * header.background.height; }
	...
	public function setData(DataList:Array<StrNameLabel>):Void
	{
		...
		if (DataList != null)
		{
			for (data in DataList)
			{
				...
				if (list != null && i <= list.length - 1 && list[i] != null)
				{
	...
	private function makeListButton(i:Int, Label:String, Name:String):FlxUIButton
	{
		...
		t.loadGraphicSlice9([FlxUIAssets.IMG_INVIS, FlxUIAssets.IMG_HILIGHT, FlxUIAssets.IMG_HILIGHT],
			Std.int(header.background.width), Std.int(header.background.height),
		...
		if (dropPanel.visible)
		{
			...
			if(list.length > 1 && canScroll)
			{
				for (swipe in FlxG.swipes)
				{
					...
					{
						var angle = FlxMath.angleBetweenPoints(swipe.startPosition, swipe.endPosition);
			...
			if (FlxG.mouse.justPressed && !FlxG.mouse.overlaps(this)) showList(false);
			#end
		}
		#end
	}
...
class FlxUIDropDownHeader extends FlxUIGroup
{
	...
	public function new(Width:Int = 120, ?Background:FlxSprite, ?Text:FlxUIText, ?Button:FlxUISpriteButton)
	{
		...
		text = Text;
		button = Button;
		if (background == null) background = new FlxUI9SliceSprite(0, 0, FlxUIAssets.IMG_BOX, new Rectangle(0, 0, Width, 20), [1,1,14,14]);
		if (button == null)
		{
		...
		button.x = background.x + background.width - button.height;
		button.width = Width;
		button.offset.x -= (Width - button.frameWidth);
		button.label.offset.x += button.offset.x;
		if (text == null) text = new FlxUIText(0,0,Std.int(background.width));
